...
public class MasterPolicy extends BaseLayout {
    ...
    public MasterPolicy() {
     ...
     // Create a layout to hold the "Uploaded documents" label and the grid together
        ...
        Button remove = new Button("Remove Master policy");
        remove.getElement().getStyle().set("color", "white");
        remove.addClassName("remove");

        // Add all components to the main layout
        VerticalLayout mainLayout = new VerticalLayout();
        mainLayout.add(program, programDetails, contract, contractDetails, policy, documentsLayout,remove);
    ...
    private Grid<PolicyDocument> createDocumentGrid() {
        ...
        documents.add(new PolicyDocument("Complete policy", "Zurich_Policy_Instruction_Form", 1));
    ...
    public class PolicyDocument {
        ...
        public PolicyDocument(String type, String name, int order) {
            this.type = type;
            this.name = name;
            this.order = order;
            this.uploadedFile = name + ".pdf";
        }
